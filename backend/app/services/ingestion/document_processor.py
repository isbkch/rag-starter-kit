        ...
        async def wrapper(*args, **kwargs):
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    logger.warning(f"Attempt {attempt + 1} failed for {func.__name__}, retrying in {wait_time:.1f}s: {e}")
...
class DocumentProcessor:
    ...
    async def _extract_text(self, document: Document) -> str:
        ...
        try:
            ...
            if text_length < 10:
                logger.warning(f"Very short text extracted ({text_length} chars) from {document.filename}")
        ...
        try:
            ...
            for chunk in chunks:
                if chunk.content and len(chunk.content.strip()) >= 5:
                    ...
                    logger.warning(f"Skipping invalid chunk {chunk.id} (too short or empty)")
    ...
    async def _generate_embeddings(self, chunks: List[DocumentChunk]) -> List[DocumentChunk]:
        ...
        try:
            ...
            for i in range(0, len(chunk_texts), batch_size):
                ...
                logger.debug(f"Processing embedding batch {i // batch_size + 1}/{(len(chunk_texts) + batch_size - 1) // batch_size}")
                
                try:
                    batch_embeddings = await embedding_service.get_embeddings(batch_texts)
                    if not batch_embeddings or len(batch_embeddings) != len(batch_texts):
                        raise EmbeddingError(
                            f"Embedding batch returned incorrect number of embeddings",
                            {
                                ...
                                "received": len(batch_embeddings) if batch_embeddings else 0,
                    ...
                    raise EmbeddingError(
                        ...
                        {"batch_index": i // batch_size, "batch_size": len(batch_texts)}
                    )
            ...
            logger.info(f"Successfully generated embeddings for {len(successful_chunks)} chunks")
        ...
        try:
            ...
            for chunk in chunks:
                if chunk.embedding and len(chunk.embedding) > 0:
                    ...
                    logger.warning(f"Chunk {chunk.id} missing embedding, skipping storage")
            ...
            raise VectorStorageError(
                f"Unexpected error storing chunks: {e}",
                {"original_error": str(e)}
            )
        finally:
    ...
    def validate_document(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                validation_result['file_info']['file_extension'] = file_path_obj.suffix.lower()
            except Exception as e:
                validation_result['is_valid'] = False
            ...
            try:
                with open(file_path, 'rb') as f:
                    ...
                    if not header:
                        validation_result['errors'].append('File appears to be empty or unreadable')
                        validation_result['is_valid'] = False
            ...
            if document_type == DocumentType.PDF:
                ...
                validation_result['warnings'].extend(docx_validation.get('warnings', []))
