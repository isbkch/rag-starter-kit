...
class VectorSearchEngine:
    ...
    async def search(
        ...
        try:
            ...
            provider = getattr(self.vector_db, "__class__", {}).get(
                "__name__", "unknown"
            )
            metrics.record_vector_operation(
                "search", provider, search_time, success=True
            )
            ...
            metrics.record_vector_operation(
                "search", provider, search_time, success=False
            )
        ...
        try:
            ...
            for doc in documents:
                ...
                if "document_id" not in vector_doc["metadata"]:
                    vector_doc["metadata"]["document_id"] = doc.get(
                        "document_id", doc.get("id", "")
                    )
                if "chunk_id" not in vector_doc["metadata"]:
                    vector_doc["metadata"]["chunk_id"] = doc.get(
                        "chunk_id", doc.get("id", "")
                    )
