...
def trace_document_processing(
    operation: str, document_id: str = None, filename: str = None
):
    ...
    return trace_function(operation_name=f"document.{operation}", attributes=attributes)


def trace_vector_operation(
    operation: str, provider: str = None, collection: str = None
):
    ...
    return trace_function(operation_name=f"vector.{operation}", attributes=attributes)
...
def shutdown_tracing():
    ...
    try:
        ...
        if trace.get_tracer_provider():
            for (
                processor
            ) in trace.get_tracer_provider()._active_span_processor._span_processors:
