    ...
    try:
        ...
        if not request.include_detailed:
            ...
            if "vector_search" in stats:
                simplified_stats["total_documents"] = stats["vector_search"].get("indexed_documents", 0)
            
            if "keyword_search" in stats:
                simplified_stats["keyword_index_size"] = stats["keyword_search"].get("indexed_documents", 0)
...
async def reindex_all_documents(
    ...
    try:
        ...
        return {
            ...
            "status": "started",
            "job_id": "reindex_" + str(hash(str(id(search_manager))))  # Placeholder job ID
        }
...
async def update_hybrid_weights(
    vector_weight: float = Query(..., ge=0.0, le=1.0, description="Vector search weight"),
    keyword_weight: float = Query(..., ge=0.0, le=1.0, description="Keyword search weight"),
    search_manager: SearchManager = Depends(get_search_manager_dep)
):
