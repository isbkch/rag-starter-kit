    ...
    try:
        ...
        try:
            ...
            background_tasks.add_task(
                search_manager.index_documents, documents_for_indexing
            )
        ...
        if "document" in locals():
            document_service.update_document_status(
                str(document.id), "failed", error_message="HTTP error during processing"
            )
        raise
        ...
        if "document" in locals():
            document_service.update_document_status(
                str(document.id), "failed", error_message=str(e)
            )
...
async def delete_document(
    ...
    try:
        ...
        if chunk_ids:
            background_tasks.add_task(
                _remove_from_search_indices, search_manager, chunk_ids
            )
...
async def reindex_document(
    ...
    try:
        ...
        if not chunks:
            raise HTTPException(
                status_code=400, detail="Document has no chunks to reindex"
            )
...
async def _remove_from_search_indices(
    search_manager: SearchManager, chunk_ids: List[str]
):
